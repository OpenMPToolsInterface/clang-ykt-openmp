...
my $opts = {
    ...
    "ompd-support"    => { targets => "rtl",               base => 0, parms => { off     => "", on        => "*"              }, suffix => sub { $_[ 0 ] eq "on" ? "ompd" : "" } },
...
my @all_opts   = qw{ target version lib-type link-type mode omp-version coverage stats ompd-support ompt-support ompt-blame ompt-trace };
